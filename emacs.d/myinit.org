...
#+end_src
**** node container type
#+name: roam-node-container-type
#+begin_src elisp :tangle no
  (cl-defmethod org-roam-node-container-type ((node org-roam-node))
    (when-let ((res (org-roam-db-query
                     [:select properties
                      :from nodes
                      :where (and (= level 0)
                                  (= file $s1))]
...
#+begin_src emacs-lisp :tangle no
  (cl-defmethod org-roam-node-type ((node org-roam-node))
    ...
    (or (org-roam-node-container-type node)
        (condition-case nil
