...
#+BEGIN_SRC emacs-lisp :tangle no
  (defun vimacs/org-narrow-to-subtree ()
    (interactive)
    (vimacs/org-narrow-to-subtree--inner))

  (defun vimacs/org-narrow-to-subtree--inner (&optional level)
    (let ((org-indirect-buffer-display 'current-window)
      ...
      (if (not (boundp 'org-indirect-buffer-file-name))
          (let ((above-buffer (current-buffer))
            ...
            (org-tree-to-indirect-buffer level)
...
#+begin_src elisp
  (defun aragaer--narrow-to-org-roam-node ()
    ...
    (let* ((node (org-roam-node-at-point))
           (node-level (and node (org-roam-node-level node))))
      (vimacs/org-narrow-to-subtree--inner node-level)))
#+end_src
*** use-package
#+begin_src emacs-lisp :noweb yes
