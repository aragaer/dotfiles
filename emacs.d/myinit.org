...
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  (defun aragaer--get-narrow-point ()
    (unless (org-before-first-heading-p)
      (save-excursion
        (org-back-to-heading t)
        (point))))

  (defun aragaer--narrow-buffer-name (&optional point)
    (if-let* ((point (or point (aragaer--get-narrow-point)))
              (elt (org-element-at-point point))
              (title (org-element-property :raw-value elt)))
        (concat (buffer-name) " -> " title)
      (buffer-name)))

  (defun aragaer--toggle-narrow (&optional point no-switch)
  ...
  Otherwise if point is set it must be on an org heading."
    (if (buffer-narrowed-p)
        (let ((curbuf (current-buffer))
          ...
          (when last-ref-p
            (kill-buffer curbuf)))
      (when-let* ((point (or point (aragaer--get-narrow-point)))
                  (new-buffer-name (aragaer--narrow-buffer-name point))
                  (curpoint (point)))
        (switch-to-buffer
    ...
    (let ((node (org-roam-node-at-point)))
      ...
      (when (buffer-narrowed-p)
        (setq-local bookmark-make-record-function
                    #'aragaer--roam-node-make-bookmark-record))))
#+end_src
...
** load local settings
#+BEGIN_SRC emacs-lisp
  (dolist (file '("local_settings.el"))
    (when-let (local-file (locate-user-emacs-file file))
      (and (file-exists-p local-file) (load local-file))))
  (dolist (file '("local.org" "my-org.org" "private.org.gpg"))
    (when-let (local-file (locate-user-emacs-file file))
      (and (file-exists-p local-file) (aragaer--load-config local-file))))
#+END_SRC
